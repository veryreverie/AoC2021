...
namespace Day14{
  ...
  std::istream& operator>>(std::istream& is, InsertionRule& insertion_rule){
    is >> insertion_rule.pair >> Common::Ignore(4) >> insertion_rule.insert;
  ...
  long long Pairs::element_count_range() const {
    ...
    [[maybe_unused]] auto [min_element, min_count] = *Common::min_map_value(element_counts);
    [[maybe_unused]] auto [max_element, max_count] = *Common::max_map_value(element_counts);
