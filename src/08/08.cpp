...
namespace Day08{
  ...
  int Display::unique_outputs() const {
    ...
    for (const auto& digit : output){
      if (Common::item_in_list(digit.size(), unique_lengths)){
        result++;
      }
    }
    return result;
  ...
  std::map<int,std::string> Display::get_num_to_rep() const {
    ...
    for (const char& c : real_to_rep["bcdf"]){
      if (not Common::item_in_list(c, real_to_rep["cf"])){
    ...
    for (const auto& rep : zero_six_nine){
      for (const char& c : wires){
        if (not Common::item_in_list(c, rep)){
          if (Common::item_in_list(c, real_to_rep["cf"])){
            num_to_rep[6] = rep;
            real_to_rep["c"] = c;
          } else if (Common::item_in_list(c, real_to_rep["bd"])){
            num_to_rep[0] = rep;
            real_to_rep["d"] = c;
          } else {
            num_to_rep[9] = rep;
    ...
    for (const auto& rep : two_three_five){
      if (Common::item_in_list(real_to_rep["e"][0], rep)){
        num_to_rep[2] = rep;
      } else if (not Common::item_in_list(real_to_rep["c"][0], rep)){
        num_to_rep[5] = rep;
      } else {
        num_to_rep[3] = rep;
      }
