...
namespace Day13{
  ...
  std::istream& operator>>(std::istream& is, Symmetry& symmetry){
    return is >> Common::Ignore(11) >> symmetry.axis >> Common::Ignore(1,'=') >> symmetry.offset;
  }
  ...
  std::istream& operator>>(std::istream& is, Dot& dot){
    return is >> dot.x >> Common::Ignore(1,',') >> dot.y;
  }
  ...
  std::ostream& operator<<(std::ostream& os, const Dotboard& dotboard){
    ...
    for (int y=0; y<=max_y; ++y){
      for (int x=0; x<=max_x; ++x){
        if (Common::item_in_list(std::tuple(x,y), dotboard.dotboard)) {
          os << '#';
        } else {
          os << '.';
        }
